    ...
    if (!isLoggedIn) {
        ...
        content = (
            <section className="w-4/5 max-w-7xl mx-auto py-10 flex flex-col items-center gap-10">
                <h2 className="text-5xl font-light tracking-wider">Add New Unpublished Post</h2>
                <form className="flex flex-col gap-3 w-full">
                    <input type='text' className="p-3 bg-slate-200" value={post_title} onChange={onTitleChange} placeholder="Post Title"></input>
                    <textarea value={post_text} className="resize-none p-3 bg-slate-200" onChange={onTextChange} placeholder="Post Text"></textarea>
                    <button type='button' className="text-3xl font-light tracking-wider" onClick={attemptNewPost} disabled={!canSubmit}>Submit</button>
                </form>
            </section>
        );
    }
